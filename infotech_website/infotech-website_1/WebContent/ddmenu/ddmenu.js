...
function Ddmenu(k) {
	...
	var r = function(a, b) {
		...
		if (a.preventDefault)
			...
			a.returnValue = false
	}, i, e, v, g = document, m = "className", a = "length", z = "addEventListener", lb = [
			"$1$2$3", "$1$2$3", "$1$24", "$1$23", "$1$22" ], B = "offsetWidth", C = "zIndex", j = "onclick", b = [], x = -1, l = 0, H = function(
			a) {
		if (l)
			l[e][v] = a ? "block" : "none"
	}, f, nb, c, h = function() {
		...
		if (b)
			...
			J(a, "over");
		a[e][C] = b ? 2 : 1
	}, cb = "ontouchstart" in window || window.DocumentTouch
			&& document instanceof DocumentTouch, T = (o.msPointerEnabled || o.pointerEnabled)
			&& (o.msMaxTouchPoints || o.maxTouchPoints);
	if (T)
		if (o.msPointerEnabled)
			var O = "MSPointerOver", P = "MSPointerOut";
		else {
			O = "pointerover";
			P = "pointerout"
		}
	var n = function(e) {
		...
		return d
	}, A = "createElement", hb = function(g, b) {
		var d = function(b) {
	...
	S.prototype = {
		...
		f : function() {
			...
			if (!h() && F(this.a, "position") == "static")
				this.a.dd[e].top = this.a.offsetTop + this.a.offsetHeight
						+ "px";
			else
				this.a.dd[e].top = ""
		},
		...
		k : function() {
			var c = this, b = this.a, g = y(b), k = g[a];
			if (s < 7)
				if (d(g[0], "top-heading"))
					g[0][e].zoom = 1;
			while (k--)
				if (d(g[k], "dropdown"))
					var i = g[k];
			if (i) {
				d(g[0], "top-heading")
						&& g[0].setAttribute("aria-haspopup", "true");
				c.i(i);
				b.dd = i;
	...
	V.prototype = {
		...
		b : function(j) {
			...
			for (var q = y(j), m = 0, r = q[a]; m < r; m++)
				q[m].nodeName == "LI" && b.push(new S(q[m]));
			(new Function(
					"a",
					"b",
					"c",
					...
					.apply(this, [ f, l, L, kb, h, c, t, lb, j, 0, i ]);
			!G && f.h && k.license.length == 6 && p(g, "keydown", ab);
			Z(j)
		}
	};
	function Z(d) {
		if (typeof d[e].webkitAnimationName != "undefined")
			var b = "-webkit-";
		else if (typeof d[e].animationName != "undefined")
			b = "";
		else
			return;
		var h = "@" + b
	...
	function bb(b, a) {
		return !a || a.nodeType != 1 ? 0 : a[i] == b ? 1
				: a[i] && a[i][i] == b ? 1 : 0
	}
	function D(a) {
		n(0);
	...
	var W = function(b) {
		...
		a.onreadystatechange = function() {
			if (a.readyState == 4 && a.status == 200) {
				var d = a.responseText, f = /^[\s\S]*<body[^>]*>([\s\S]+)<\/body>[\s\S]*$/i;
				if (f.test(d))
					d = d.replace(f, "$1");
				var c = t(A, "div");
		...
		if (g[z])
			g[z]("DOMContentLoaded", b, false);
		else
			p(window, "load", b)
	};
	if (s < 9) {
		var jb = t(A, "nav"), U = r(g, "head");
		if (!U[a])
			return;
		U[0].appendChild(jb)
	}
