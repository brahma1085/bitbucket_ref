...
public class DepositPanel extends JPanel implements FocusListener,KeyListener,ItemListener,Observer,ActionListener
{

	GridBagConstraints gbc;
	static DepositPanel deposit;
	...
	JTextField[] txt_array = new JTextField[12]; //{ txt_ac_no ,txt_cid, txt_deposit_dt, txt_matur_dt,txt_deposit_days,txt_deposit_amt,txt_matur_amt,txt_next_pay_dt ,txt_intrest_rt ,txt_int_upto_dt,txt_reciept,txt_int_paid_dt } ;
	String[] txt_names = { "Account No", "Cid","Deposit Date","Maturity Date","Deposit Days"," Deposit Amount","Next pay date ","Interest Rate","Interest Upto Date"," Reciept No","Interest Paid Date"};
	...
	private DepositPanel() {
		...
		addComponent(new JLabel("Deposit Peroid"), this,  5, 1, 0.0, 0.0, 3, 1);
		...
		addComponent(new JLabel("Deposit Amt"), this, 7, 1, 0.0, 0.0, 3, 1);
    ...
    public static DepositPanel getInstance(int i) {
		
    	if ( i == 0)
    		return	deposit = new DepositPanel();
		...
		if ( ar.getSource() == txt_ac_no  ) {
			...
			if ( txt_ac_no.getText().toString().trim().length() > 0 &&  new Integer(txt_ac_no.getText().toString().trim()).intValue() >0 ) {
				...
				try {
					...
					Object[] obj = store_dep.getDepositAccount(combo_ac_type.getSelectedIndex() == 0 ? "1003001":"1004001", new Integer(txt_ac_no.getText().toString().trim()).intValue());
