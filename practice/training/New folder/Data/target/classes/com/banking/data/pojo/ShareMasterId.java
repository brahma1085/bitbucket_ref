package com.banking.data.pojo;
// default package



/**
 * ShareMasterId entity. @author MyEclipse Persistence Tools
 */

public class ShareMasterId  implements java.io.Serializable {


    // Fields    

     private String acType;
     private Integer acNo;
     private Integer tempNo;


    // Constructors

    /** default constructor */
    public ShareMasterId() {
    }

    
    /** full constructor */
    public ShareMasterId(String acType, Integer acNo, Integer tempNo) {
        this.acType = acType;
        this.acNo = acNo;
        this.tempNo = tempNo;
    }

   
    // Property accessors

    public String getAcType() {
        return this.acType;
    }
    
    public void setAcType(String acType) {
        this.acType = acType;
    }

    public Integer getAcNo() {
        return this.acNo;
    }
    
    public void setAcNo(Integer acNo) {
        this.acNo = acNo;
    }

    public Integer getTempNo() {
        return this.tempNo;
    }
    
    public void setTempNo(Integer tempNo) {
        this.tempNo = tempNo;
    }
   



   public boolean equals(Object other) {
         if ( (this == other ) ) return true;
		 if ( (other == null ) ) return false;
		 if ( !(other instanceof ShareMasterId) ) return false;
		 ShareMasterId castOther = ( ShareMasterId ) other; 
         
		 return ( (this.getAcType()==castOther.getAcType()) || ( this.getAcType()!=null && castOther.getAcType()!=null && this.getAcType().equals(castOther.getAcType()) ) )
 && ( (this.getAcNo()==castOther.getAcNo()) || ( this.getAcNo()!=null && castOther.getAcNo()!=null && this.getAcNo().equals(castOther.getAcNo()) ) )
 && ( (this.getTempNo()==castOther.getTempNo()) || ( this.getTempNo()!=null && castOther.getTempNo()!=null && this.getTempNo().equals(castOther.getTempNo()) ) );
   }
   
   public int hashCode() {
         int result = 17;
         
         result = 37 * result + ( getAcType() == null ? 0 : this.getAcType().hashCode() );
         result = 37 * result + ( getAcNo() == null ? 0 : this.getAcNo().hashCode() );
         result = 37 * result + ( getTempNo() == null ? 0 : this.getTempNo().hashCode() );
         return result;
   }   





}