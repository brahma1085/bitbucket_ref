...
<%@page import="masterObject.backOffice.OpenedClosedInoperatedAccountObject"%>
<html>
<head>
<meta http-equiv="Content-Type" content="text/html; charset=ISO-8859-1">
<title>OpenedClosedAccountsPrintingReport</title>
...
function ValidateForm(){
 ...
 // alert("From day is greater than To day !!!! Enter valid date")
  ...
  if(document.getElementById("rcdext").value!=null)
       {
       	if(document.getElementById("rcdext").value=="recexist")
       	{
...
<% String date=(String)request.getAttribute("date");
   String[] accstatus=(String[])request.getAttribute("AccStatus");
   ModuleObject[] modcode=(ModuleObject[])request.getAttribute("AccTypes");
   
%>
<%if(access!=null&&accesspageId!=null&&access.toCharArray()[0]=='1'){%>
<html:form action="/BackOffice/OpenedClosedAccountsPrintingReport?pageId=11005">
<center><h2 class="h2">Opened/Closed Accounts</h2></center><br>
...
<table class="txtTable">
 	<tr>
		...
		<td><bean:message key="label.to_dt"></bean:message><html:text property="todate" size="10" onblur="ValidateForm()"  onchange="properdate(fromdate.value,todate.value)"   onkeypress="return numbersonly_date(this)" styleClass="formTextFieldWithoutTransparent"></html:text></td>
   </tr>
 </table>
 <br>
<table class="txtTable">
    <tr>
 		<td><bean:message key="lable.accstatus"></bean:message>
     		<html:select property="acc_status"  >
     		  <%for(int i=0;i<accstatus.length;i++) {%>
     			<html:option value="<%=accstatus[i]%>"><%=accstatus[i]%></html:option>
     			<%} %>
     		</html:select> 
 		</td>
 		<td></td><td></td><td></td><td>
		<td><bean:message key="lable.acctye"></bean:message></td>
	 		<td><html:select property="acc_type" size="1">
...
<br><br>
 <table class="txtTable">  
   <tr>
	    ...
	    <html:hidden property="records" styleId="rcdext"/>
...
<center>
 ...
 <div  id = "table1" style="display: block;width: 750px;height: 300px">
   			<display:table name="OpenedClosedAccounts" export="true" id="currentRowObject" class="its" sort="list" requestURI="/BackOffice/OpenedClosedAccountsPrintingReport.do" pagesize="10">
   			<display:setProperty name="export.rtf.filename" value="example.rtf"/>
   			<display:column media="csv excel" title="URL" property="url" />
    		<display:setProperty name="export.pdf" value="true" /> 
 
 
   <display:column title="AC Type" property="modAbrr" style="background-color:#F2F0D2"></display:column>
   <display:column title="AC No" property="accountNo" style="background-color:#F2F0D2"></display:column>
   <display:column title="Name" property="accountName" style="background-color:#F2F0D2"></display:column>
   <display:column title="Address" property="accountAddress" style="background-color:#F2F0D2"></display:column>
   <display:column title="A/C Category" property="accountCategory" style="background-color:#F2F0D2"></display:column>
   <display:column title="Opening Dt" property="openDate" style="background-color:#F2F0D2"></display:column>
   <display:column title="Closing Dt" property="closeDate" style="background-color:#F2F0D2"></display:column>
   <display:column title="Remarks" style="background-color:#F2F0D2"></display:column>  
