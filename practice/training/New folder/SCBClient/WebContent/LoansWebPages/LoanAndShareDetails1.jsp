...
<%
	if(sharedet!=null)
	{ 
	 for(int i1=1;i1<sharedet.length;i1++)
 	 {
		 System.out.println("m"); 
		System.out.println("LoanAvail====<"+amt);   
 	  amt = (Double.parseDouble(sharedet[i1][3].toString()))+ Double.parseDouble(sharedet[i1][4].toString());
  	  loan_amt+=Double.parseDouble(sharedet[i1][4].toString());
  	  share_amt+= amt;
  	  sanc_amt+=Double.parseDouble(sharedet[i1][2].toString());
      disb_amt+=Double.parseDouble(sharedet[i1][3].toString()); 
     }
