...
<h2 class="h2"><center>Agent Remittance Report</center></h2>
     
    <script type="text/javascript">
         function set(target){
         if(document.forms[0].agent_code.value=="SELECT")
         {
         alert('Select Agent Type');
         }
         else if(document.forms[0].agent_no.value=="")
         {
...
<body class="Mainbody">
		...
		<%
			...
			ptran=(PygmyTransactionObject[])request.getAttribute("AgRemittance");
...
<%if(access!=null&&accesspageId!=null&&access.toCharArray()[0]=='1'){%>
		<html:form action="/Pygmy/AgentRemittanceReport?pageid=8006">
		<table class="txtTable">
			<td>
				<tr>
                     <td><bean:message key="label.agentcode"></bean:message></td>
                     <td><html:select  property="agent_code" styleClass="formTextFieldWithoutTransparent">
                         <html:option value="SELECT">SELECT</html:option>
                         <% array_module=(ModuleObject[])request.getAttribute("AgentAcType");
 							System.out.println("here i am=="+array_module); %>                       
							<core:forEach var="agent_code" items="${requestScope.AgentAcType}" varStatus="count">
								<html:option value="${agent_code.moduleCode}">${agent_code.moduleAbbrv}</html:option>
                        	</core:forEach>
                         </html:select></td>
                </tr>
                ...
                <tr>
                	<td><bean:message key="label.remittance_date"></bean:message></td>
                	<td><html:text property="rem_date" size="10"  styleClass="formTextFieldWithoutTransparent" onblur="return datevalidation(this)" onkeypress="return numbersonly_date(this)"></html:text></td>
				</tr>
				...
				<tr>
					...
					<%if(access!=null&&accesspageId!=null&&access.toCharArray()[0]=='1'){%>
					<td><html:button property="sun" onclick="set('view')" value="View" styleClass="ButtonBackgroundImage">View</html:button>
             		<%}else{ %>
	...
	<%if(ptran!=null){ %>
			<div  id = "table1" style="display: block;overflow:scroll;width: 750px;height: 300px">
   			<display:table name="AgRemittance"  id="currentRowObject" class="its" >
   			 		<display:column property="accType"  title="A/C Type"  ></display:column>
                    <display:column property="accNo"  title="A/C No"  ></display:column>
            	    <display:column property="name"  title="Name" ></display:column>
                    <display:column property="collectionDate"  title="Collection Date" ></display:column>
                    <display:column property="tranAmt"  title="Remittance amount" ></display:column>
               </display:table>
        	</div>
		<%} %>		
