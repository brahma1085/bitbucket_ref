<%@ taglib uri="/WEB-INF/struts-html.tld" prefix="html" %>
<%@ taglib prefix="bean" uri="http://struts.apache.org/tags-bean" %>
<%@ taglib prefix="core" uri="/WEB-INF/c-rt.tld" %>    
<%@ taglib prefix="c" uri="http://java.sun.com/jstl/core" %>
    
<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<%@page import="masterObject.general.ModuleObject" %>
<%@page import="masterObject.clearing.Reason" %>

<%@page import="java.util.Map;"%>
<html>
<head>
<link rel="stylesheet" type="text/css" href="label.css" />
<meta http-equiv="Content-Type" content="text/html; charset=ISO-8859-1">

<script type="text/javascript">
	
	function submitMICR(id)
	{
 	
 		if(document.getElementById("citycode").value.length != 3)
 		{
				document.getElementById("err_mess_id").value= "Enter the City Code";
				
				document.getElementById("citycode").focus();
				
				return false;
		} 
		else if(document.getElementById("citycode").value == '000')
 		{
				document.getElementById("err_mess_id").value= "Enter Valid City Code" ;
				document.getElementById("citycode").focus();
				return false;
		} 
		else if(document.getElementById("bank_code").value.length != 3)
		{
				document.getElementById("err_mess_id").value="Enter the Bank Code";
				document.getElementById("bank_code").focus();
				return false;
		}
		else if(document.getElementById("bank_code").value == '000')
		{
				document.getElementById("err_mess_id").value= "Enter Valid Bank Code" ;
				document.getElementById("bank_code").focus();
				return false;
		} 
		else if(document.getElementById("branch_code").value.length != 3)
		{
				document.getElementById("err_mess_id").value="Enter the Branch Code";
				document.getElementById("branch_code").focus();
				return false;
		} 
		else 
		{                              
				document.forms[0].form_flag.value = 1;
			 	document.forms[0].submit();				
				return true;
		}
	
	}
	function calls(input)
	{
			var index = -1, i =0, found = false ;
	
			while(i< input.form.length && index==-1)
			{
				if(input.form[i] == input )
					index = i;
				else
					i++; 
			} 
		alert(input.form[4].value);
	}

	function call()
	{
	
		if(document.getElementById("credit_acc_type" ).value.substring(0,4) == "1016")
		{
			select_ChqPo ();
		} 	
		if(document.getElementById("int_clr").checked)
		{
		
				document.getElementById("citycode").disabled=true;
				document.getElementById("bank_code").disabled=true;
				document.getElementById("branch_code").disabled=true;
				document.getElementById("branchname").value=1;
				document.getElementById("branchname").disabled=true;
				document.getElementById("chequeno").disabled=true;
				document.getElementById("chqdate").disabled=true;
				
			document.getElementById("div_debit_pan").style.display = 'block';
		}
		if(document.getElementById("bounce_id").checked)
		{
			callBounce();
		} 
			checkError();	
			check_discount();
			check_laon();
		 	check_multi();
		   setcheckflag();
	}
	function setCompany(flag_num)
	{
		document.getElementById("flagset").value=flag_num;
		document.forms[0].submit();
	
	}
	function setflag(flag_num,ids)
	{
		if(ids.value!= "")
		{
			
			if(flag_num == 3) 
			{
				if((document.getElementById("debit_acc_num").value == document.getElementById("credit_acc_no").value ) && (document.getElementById("credit_acc_type").value == document.getElementById("debit_acc_type").value))
				{
					document.getElementById("err_mess_id").value="Credit Account and Debit Account are Same plz verify ";
					document.getElementById("credit_acc_no").value=0;
					return false;
				}
			} 
			else if(flag_num == 5) 
			{
				if((document.getElementById("credit_acc_no").value == ids.value) && (document.getElementById("credit_acc_type").value == document.getElementById("debit_acc_type").value))
				{
					document.getElementById("err_mess_id").value="Credit Account and Debit Account are Same plz verify " ;
					document.getElementById("debit_acc_num").value=0;
					return false;
				}
				
			} 
			else if(flag_num==8) 
			{
				if(document.getElementById("ctrl_no").value != 0)
				{
					document.getElementById("err_mess_id").value="Control Number Should Be Zero";
					return false;
				}
				else if(document.getElementById("citycode").value.length != 3 )
				{
					document.getElementById("err_mess_id").value="Enter the City Code";
					document.getElementById("citycode").focus();
					return false;
				}
				else if(document.getElementById("bank_code").value.length != 3)
				{
					document.getElementById("err_mess_id").value="Enter the Bank Code";
					document.getElementById("bank_code").focus();
					return false;
				}
				else if(document.getElementById("branch_code" ).value.length != 3 )
				{
						document.getElementById("err_mess_id").value="Enter the Branch Code" ;
						document.getElementById("branch_code").focus();
						return false;
				}
				else if(document.getElementById("chk_comp").checked)
				{
						if(document.getElementById("CompName").value == "")
						{
							document.getElementById("err_mess_id").value="Enter Valid Company Name";
							document.getElementById("CompName").focus();
							return false;
						}
				}
				else if(document.getElementById("credit_account_no").value==0)
				{
						document.getElementById("err_mess_id").value="Enter Valid Account Number";
						document.getElementById("credit_account_no").focus();
						return false;
				}
				else if(document.getElementById("amt").value==0)
				{
						document.getElementById("err_mess_id").value="Enter Valid Cheque Amount";
						document.getElementById("amt").focus();
						return false;
				}
				else if(!(document.getElementById("int_clr").checked))
				{
					if(document.getElementById("chqdate").value == '')
					{
						document.getElementById("err_mess_id").value="Enter Valid Cheque Date";
						document.getElementById("chqdate").focus();
						return false;
					}
					else if(document.getElementById("chequeno").value == 0)
					{
							document.getElementById("err_mess_id").value="Enter Cheque Number";
							return false;
					}
				
				}
				else if(document.getElementById("int_clr").checked)
				{
						if(document.getElementById("debit_chq_no").value == 0)
						{
							document.getElementById("err_mess_id").value="Enter Cheque Number";
							return false;
						}
						else if(document.getElementById("debit_che_date").value == "")
						{
							document.getElementById("err_mess_id").value="Enter Cheque Date";
							document.getElementById("debit_che_date").focus();
							return false;
						}
						else if(document.getElementById("credit_acc_type").value==document.getElementById("debit_acc_type").value)
						{
							if(document.getElementById("credit_account_no").value==document.getElementById("debit_acc_num").value)
							{
								document.getElementById("err_mess_id").value="Both Credit And Debit Account Can't Be Same";
								return false;
							
							}	
						}
				}	
					
			}
			else if(flag_num==9)
			{
				if(document.getElementById("ctrl_no").value==0)
				{
					document.getElementById("err_mess_id").value="Enter Valid Control Number";
					document.getElementById("ctrl_no").focus;
					return false;
				}
				
			}
			else if(flag_num==10)
			{
				if(document.getElementById("ctrl_no").value==0)
				{
					document.getElementById("err_mess_id").value="Enter Valid Control Number";
					document.getElementById("ctrl_no").focus;
					return false;
				}
				
			}
			
			if(ids.id == "dis_amt")
			{
				if(document.getElementById("dis_amt").value.length  > 0 && document.getElementById("amt").value .length > 0)
				{
					if(parseInt(document.forms[0].discountamount.value) > parseInt(document.forms[0].amount.value))
					{
							document.getElementById("err_mess_id").value=" Discount Amount is greater than the Cheque Amount ";
							document.getElementById("dis_amt").value='';
							return false;
					}
				} 
				else 
				{
						document.getElementById("err_mess_id").value="please Enter The Fields ";
						return false;
				}
			}
			
			document.getElementById("today_dt").value = getTodayDate();
			document.getElementById("flagset").value = flag_num;
			document.forms[0].booleanFlag.value=0;
			document.forms[0].submit();
	
		}
		
	}
	
	function setcheckflag()
	{
			if(document.getElementById("int_clr").checked)
			{
				document.getElementById("div_debit_pan").style.display = 'block';					
				document.getElementById("citycode").disabled=true;
				document.getElementById("bank_code").disabled=true;
				document.getElementById("branch_code").disabled=true;
				document.getElementById("bankname").disabled=true;
				document.getElementById("branchname").disabled=true;
				document.getElementById("chequeno").disabled=true;
				document.getElementById("chqdate").disabled=true;
			
			} 
			else 
			{
				document.getElementById("div_debit_pan").style.display = 'none';
				document.getElementById("citycode").disabled=false;
				document.getElementById("bank_code").disabled=false;
				document.getElementById("branch_code").disabled=false;
				document.getElementById("bankname").disabled=false;
				document.getElementById("branchname").disabled=false;
				document.getElementById("chequeno").disabled=false;
				document.getElementById("chqdate").disabled=false;
				document.getElementById("debit_acc_num").value=0;
			}
			
	}
	
	function checkError()
	{
		
		if(document.getElementById("err_id").value=='1')
		{
			clearAll();
		}	
	}
	
	function check_laon(flag_num)
	{
		
		if(document.getElementById("loan_cre").checked) 
		{
			document.getElementById("loan_div").style.display = 'block';  
			document.getElementById("loan_div1").style.display = 'block';
		} 
		else 
		{
			document.getElementById("loan_div").style.display = 'none';  
			document.getElementById("loan_div1").style.display = 'none';
			
		}
	}
	
	function check_multi()
	{
		if(document.getElementById("chk_comp").checked) 
		{
			document.getElementById("div_comp1").style.display = 'block';  
			document.getElementById("div_comp").style.display = 'block';
			document.getElementById("credit_acc_type").disabled = true ; 
			document.getElementById("credit_acc_no").disabled = true ;
		}		
		else 
		{
			
			document.getElementById("div_comp1").style.display = 'none';  
			document.getElementById("div_comp").style.display = 'none';
			document.getElementById("credit_acc_type").disabled = false ;
			document.getElementById("credit_acc_no").disabled = false ;
		}
		
	}
	
	function check_discount()
	{
		if(document.getElementById("chk_discount").checked )
		 {
			document.getElementById("div_discount1").style.display = 'block';  
			document.getElementById("div_discount").style.display = 'block';
			
		 } 
		 else 
		 {
			document.getElementById("div_discount1").style.display = 'none';  
			document.getElementById("div_discount").style.display = 'none';
   		 }
	
	}
	
	function open_div_pocomm(value)
	{
		if(value)
		{
			document.getElementById("div_discount1").style.display = 'block';  
			document.getElementById("div_discount").style.display = 'block';
		} 
		else 
		{
			document.getElementById("div_discount1").style.display = 'none';  
			document.getElementById("div_discount").style.display = 'none';
		}
	
	 }
	
     function onlynumbers()
     {
        	var cha =  event.keyCode;
            if(cha >= 48 && cha <= 57) 
            {
            	return true;
            } 
            else 
            {
        		return false;
        	}
       }
        
      function onlyDoublenumbers()
      {
        	var cha = event.keyCode;
            if((cha >= 48 && cha <= 57 )||cha == 46) 
            {
            		return true;
           	} 
           	else 
           	{
        			return false;
        	}
			        
        }
        
        
        function numbersonly(input)
        {
		      var cha =   event.keyCode;

            if(cha >= 48 && cha <= 57) 
            {
					 if(input.value.length >= 2)  
					 {
							if(input == document.getElementById("branch_code"))
							{
								if(input.value.length == 2)
									input.value = input.value + String.fromCharCode(cha);
								return false ;
							}
							else 
							{			 	
								input.value = input.value + String.fromCharCode(cha);
					 			var j = getnextFeild(input);
                				input.form[j].focus();
                				return false ;
                			}
					 
					}	
                    return true;
              } 
              else 
              {
	             return false;
		      }
         }
        
	
	function getnextFeild(input)
	{
			var index  = -1;
			var i = 0 ;
			while(i< input.form.length  && index == -1)   
			{
				if(input.form[i] == input) 
				{
					index = i + 1 ;
					return index;
				} 
				else 
				{
					  i++;
				}
			}
			
		return index;
	 }	
	 function checkformat(ids) 
	 {
            var cha = event.keyCode;
            var format = true;
            var allow=true;
		    var ff =  ids.value;
            var dd = ff.split( '/' );

			if(dd.length == 3)
			{
	             for(var i =0; i< dd.length; i++)
	             {
                      if(i != 2 && dd[i].length != 2 )
                      {
                          document.getElementById("err_mess_id").value="Problem In Date Format-DD/MM/YYYY";
                          format = false ;
                          allow=false;
                      } 
                      else if(i==2 && dd[i].length != 4 )
                      {
                           document.getElementById("err_mess_id").value="Problem In Date Format-DD/MM/YYYY";
                           format = false ;
                           allow=false;
                      }
            	 }
             } 
             else
             {
                    allow=false;
             		format = false ;
             		document.getElementById("err_mess_id").value="Problem In Date Format-DD/MM/YYYY";
                          
             }

        	if(format)
        	{
       			 var date=new Date();
         		 var dayCheck=dd[0];
         
         		 var monthCheck=dd[1];
         		 var yrCheck=dd[2];
         		 var mth=dd[1];
         
        		 dd[1]=dd[1]-1;
	   			 var days = 32 - new Date(dd[2],dd[1],32).getDate();
         		if(dd[0].length==2)
         		{
          			if(dd[0] > days)
                    {
						document.getElementById("err_mess_id").value="Day should not be greater than "+days+" ";
                        allow=false;
                    }
     			    if(dd[0]==00)
     			    {
          				document.getElementById("err_mess_id").value="There is no date with 00";
         	 			allow=false;
          			}
                    if(mth<1 || mth>12)
                    {
          				document.getElementById("err_mess_id").value="Month should be greater than 0 and \n lessthan 13  "+mth+" ";
          				allow=false;
          			}
          		}
          		if(dd[2].length==4)
          		{
          			if((parseInt(dd[2])<=parseInt(date.getYear())))
          			{
          				allow=true;
                    }
          			else
          			{
          				document.getElementById("err_mess_id").value="Year should be less than "+date.getYear()+" ";
          				allow=false;
          			}
          		}
          	}
			if(allow)
			{
			  return true;
			}
			else
			{
				document.forms[0].chequedate.focus();
		 		return false;
			}
		}	
	        
        function chequeDate(ids ,eve) 
        {

              var cha =   event.keyCode;
              var format = true;
              var ff =  ids.value;

              var dd = ff.split( '/' );

	    		if(dd.length == 3)
	    		{
					for(var i =0; i< dd.length; i++ )
					{
				     if(i != 2 && dd[i].length != 2 )
                      {
					      document.getElementById("err_mess_id").value="Problem in Date Format";
                          format = false ;
                          break;

                      } 
                      else if(i==2 && dd[i].length != 4 )
                      {
					       document.getElementById("err_mess_id").value="Problem in Date Format ";
                           format = false ;
                           break;
                      }
		        	}
             	} 
             	else
             	{

             		format = false ;
             		document.getElementById("err_mess_id").value="Problem in Date Format ";
             	}
				if(format)
				{
					 	var date=new Date();
         		 		var dayCheck=dd[0];
         				var monthCheck=dd[1];
         		 		var yrCheck=dd[2];
         		 		var mth=dd[1];
         
        		 			dd[1]=dd[1]-1;
	   			 			var days = 32 - new Date(dd[2],dd[1],32).getDate();
         		
         				if(dd[0].length==2)
         				{
          					if(dd[0] > days)
                    		{
								document.getElementById("err_mess_id").value="Day should not be greater than "+days+" ";
                        		allow=false;
                    		}
     			    		if(dd[0]==00)
     			    		{
          						document.getElementById("err_mess_id").value="There is no date with 00";
         	 					allow=false;
          					}
                    		if(mth<1 || mth>12)
                    		{
          						document.getElementById("err_mess_id").value="Month should be greater than 0 and \n lessthan 13  "+mth+" ";
          						allow=false;
          					}
          				}
          				if(dd[2].length==4)
          				{
          					if((parseInt(dd[2])<=parseInt(date.getYear())))
          					{
          						allow=true;
                    		}
          					else
          					{
          						document.getElementById("err_mess_id").value="Year should be less than "+date.getYear()+" ";
          						allow=false;
          					}
          				}
					
				
        		}
        		if(allow)
				{
			 		 	return true;
				}
				else
				{
						document.forms[0].debit_chq_date.focus();
		 				return false;
				}
        		
        }
        
         function getTodayDate()
         {
        
        	var date = new Date();
         	var day = date.getDate();
        	var month = date.getMonth()+1; 
        	var year = date.getYear();
        	
		   	if(day <= 9)
        	{
        		day = "0"+day;
        	}  
        	if(month <= 9)
        	{
				month = "0"+month;        	
        	}
        	return day+"/"+month+"/"+year;
         }
       	function callBounce()
       	{
       
       			if(document.getElementById("bounce_id").checked )
		 		{
					document.getElementById("bounce_fine").style.display = 'block';  
					
				} 
				else 
				{
					document.getElementById("bounce_fine").style.display = 'none';  
				}
        }

		function clearfeild(ids)
		{
			document.getElementById(ids).value= "";
		}

	function clearAll()
	{
	
		var form_ele = document.forms[0].elements;
		
		var val = document.forms[0].error_message.value;
		
		for(var i = 0 ; i< form_ele.length; i++ )
		{
			if(form_ele[i].type=="text")
				form_ele[i].value = "";
		
		}
		
		document.getElementById("bounce_id").checked = false; 
		document.getElementById("int_clr").checked = false;
		document.getElementById("outcheque").checked = false;
		document.getElementById("chk_comp").checked = false;
		document.getElementById("loan_cre").checked = false;
		document.getElementById("div_debit_pan").style.display = 'none';
		document.getElementById("amt").value=(0.0);
		document.getElementById("chequeno").value=0;
		document.getElementById("credit_acc_no").value=0;
		document.getElementById("debit_acc_num").value=0;
		document.getElementById("ctrl_no").value=0;
		document.forms[0].error_message.value=val;
		return false;
	}
		
		
	function select_ChqPo()
	{
	
			if(document.getElementById("int_clr").checked) 
			{
				if(document.getElementById("chddpo").value == "P")
				{
					document.getElementById("div_debit_pan" ).style.display = 'none';
				} 
				else if(document.getElementById("chddpo").value == "C")
				{
							document.getElementById( "div_debit_pan" ).style.display = 'block';
							document.getElementById( "div_pocomm" ).style.display = 'none';
							document.getElementById( "div_pocomm1" ).style.display = 'none';
							document.getElementById( "po_div" ).style.display = 'none';
				} 
			 }
	
			var val = document.getElementById("credit_acc_type").value;
			if(val.substring(0,4) == "1016") 
			{
				document.getElementById("credit_acc_no").value = 0;
				document.getElementById("cr_name").value = "Pay order";
				document.getElementById( "div_pocomm" ).style.display = 'block';
				document.getElementById( "div_pocomm1" ).style.display = 'block';
			}
			
	}	
		
	function  setDiscount()
	{
	
		var cr_code = document.getElementById("credit_acc_type").value;
		
		if(cr_code.substring(0,4) == "1002" || cr_code.substring(0,4) == "1007" || cr_code.substring(0,4) == "1014" ) 
		{
				document.getElementById("chk_discount").disabled = false;
		} 
		else 
		{
				document.getElementById("chk_discount").disabled = true;
		}
		
		if(cr_code.substring(0,4) == "1003" || cr_code.substring(0,4) == "1005" )
		{
			document.getElementById("credit_acc_no").disabled = true;
		}
		else
		{
			document.getElementById("credit_acc_no").disabled = false;
		}
	
	}	
	
	
function setLoanCredit(flagVal)
{
	if(document.getElementById("loan_cre").checked)
	{
		document.getElementById("flagset").value=flagVal;
		document.forms[0].submit();
	}
	else
	{
		document.getElementById("flagset").value=0;
		document.forms[0].submit();
	}


}	
	
	
		



</script>

</head>

<body bgcolor="#edeefe" class="MainBody" onload="call()">
	<center><h2 class="h2"><bean:message key="label.ChequeDeposition"></bean:message></h2></center>

<%!
	ModuleObject[] module = null;
	ModuleObject[] module_debit = null;    
	Reason[] reason=null;
	String date=null;
%>

<%!
String access;
String  accesspageId;
 Map user_role;

%>

<%

  accesspageId=(String)request.getAttribute("accesspageId");
    user_role=(Map)request.getAttribute("user_role");

if(user_role!=null&&accesspageId!=null){
			if(user_role.get(accesspageId)!=null){
				access=(String)user_role.get(accesspageId);
				System.out.println("access-----In SBOpening------>"+access);
			}

			
			}
%>

<% 
    module = (ModuleObject[])request.getAttribute("Main_Module_code");
 	module_debit = 	(ModuleObject[])request.getAttribute("Debit_Module_code");
 	reason=(Reason[])request.getAttribute("Reason");
 	date=(String)request.getAttribute("date");
%>
<%if(access!=null&&accesspageId!=null&&access.toCharArray()[0]=='1'){%>
 <html:form action="/Clearing/ChequeDeposition?pageId=7001" styleId="kk">
 <table> 
 		<tr><html:text property="error_message" styleId="err_mess_id" readonly="true" styleClass="formTextField" style="color:red;font-family:bold;" size="75"></html:text></tr>
		<tr>
		<td valign="top">							
		<table>
			
			<tr> 
				<td><bean:message key="label.controlno"></bean:message></td>
				<td><html:text property="ctrl_no" size="8"  styleClass="formTextFieldWithoutTransparent"  onkeypress="return onlynumbers()"  onblur="return setflag(2,this)" styleId = "ctrl_no"></html:text></td>
			</tr>
			<tr>
				<td><bean:message key="label.InternalClg"></bean:message></td>
				<td><html:checkbox property="internal_clearing" styleId="int_clr" styleClass="formTextFieldWithoutTransparent" value="intlclg" onclick="setcheckflag()"></html:checkbox>
					<bean:message key="label.OutstationChq"></bean:message><html:checkbox property="outstation_chq" styleId="outcheque" styleClass="formTextFieldWithoutTransparent"></html:checkbox></td>
			</tr>	
			<tr>
				<td><bean:message key="label.microcde"></bean:message></td>
				<td><html:text property="city_code" onkeypress="return numbersonly(this)" styleClass="formTextFieldWithoutTransparent" onfocus="clearfeild(this.id)" size="4" styleId="citycode"></html:text>-<html:text property="bank_code" onkeypress="return numbersonly(this)" onfocus="clearfeild(this.id)" size="4" styleId="bank_code"></html:text>-<html:text property="branch_code" size="4" onkeypress="return numbersonly(this)" onblur="return submitMICR(this)" onfocus="clearfeild(this.id)" styleId="branch_code"></html:text><html:text property="cityname"  styleClass="formTextField"  readonly="true" style="color:blue"></html:text>
					<html:hidden property="micrcode" styleId="micr"></html:hidden>	
				</td>
			</tr>
				
			<tr>
				<td><bean:message key="label.bankName"></bean:message></td>
				<td><html:text property="bankname" styleClass="formTextField" readonly="true" style="color:blue"></html:text></td>
			</tr>
			
			<tr>
				<td><bean:message key="label.branch_name"></bean:message></td>
				<td><html:text property="branchname" styleClass="formTextFieldWithoutTransparent"></html:text></td>
			</tr>
			
			<tr>
				<td><bean:message key="label.multi_credit"></bean:message></td>
				<td><html:checkbox property="multicredit" value="Yes" styleClass="formTextFieldWithoutTransparent" onclick="check_multi()"  styleId="chk_comp"></html:checkbox>
					<bean:message key="label.LoanCredit"></bean:message><html:checkbox property="laon_credit" styleClass="formTextFieldWithoutTransparent" value="loan" onclick="setLoanCredit(16)" styleId="loan_cre"></html:checkbox>
				</td>
			</tr>
					
			<tr>
				<td><bean:message key="label.acctype"></bean:message></td>
				<td>
					   <html:select property="credit_account_type"  styleClass="formTextFieldWithoutTransparent" styleId="credit_acc_type" onchange="setDiscount()" >
							<html:option value="select">Select</html:option>
								<core:forEach var="module"  varStatus="count" items="${requestScope.Main_Module_code}" >
									<html:option value="${module.moduleCode}"><core:out value="${module.moduleAbbrv}"></core:out></html:option>
								</core:forEach>
						</html:select>
						<html:text property="credit_account_no" onkeypress="return onlynumbers()" styleClass="formTextFieldWithoutTransparent" onblur="return setflag(3,this)" size="6" styleId="credit_acc_no" ></html:text>
						<html:text property="credit_acc_type_name"  styleClass="formTextField"  styleId="cr_name"  style="color:blue" readonly="true"></html:text> 
				</td>
			</tr>
	
			<tr>
				<td><div id="loan_div1" style="display:none;"><bean:message key="label.LoanCredit"></bean:message></div></td>		
		  		<td><div id="loan_div" style="display:none;">
					<html:select property="laon_credit_ac_type" styleClass="formTextFieldWithoutTransparent" >
						<html:option value="select">Select</html:option>
						<core:forEach var="module"  varStatus="count" items="${requestScope.Loan_Module_code}" >
							<html:option value="${module.moduleCode}"><core:out value="${module.moduleAbbrv}"></core:out></html:option>
					</core:forEach>
					</html:select> 
					<html:text property="laon_credit_no" size="9"  styleClass="formTextFieldWithoutTransparent"  onkeypress="return onlynumbers()" onblur="return setflag(15,this)"></html:text> 
					<html:text property="loan_acc_name" styleClass="formTextField" style="color:blue" readonly="true"></html:text>
					</div>
				</td>
			</tr>
	
			<tr>
				<td>
				<div id="div_comp1" style="display: none;">
					<bean:message key="label.companyCode"/>
				</div>
				</td>
				<td>
				<div  id="div_comp" style="display: none;" >
				<html:text property="company_name" onkeypress="return onlynumbers()" styleId="CompName" onblur="setCompany(14)" styleClass="formTextFieldWithoutTransparent"></html:text><html:text property="cname" readonly="true" style="color:blue" styleClass="formTextField"></html:text>
				</div>
				</td>
			</tr>
	
			<tr>
				<td><bean:message key="label.amount"/></td>
				<td><html:text property="amount" styleClass="formTextFieldWithoutTransparent" onkeypress="return onlyDoublenumbers()"   onblur="return setflag(13,this)" styleId="amt"></html:text></td>
			</tr>
	
			<tr>
				<td><div id="div_pocomm" style="display:none"><bean:message key="label.pocomm"/></div></td>
				<td><div id="div_pocomm1" style="display:none"><html:text property="pocommision" styleClass="formTextFieldWithoutTransparent"></html:text></div></td>
			</tr>
	
			<tr>
				<td><bean:message key="label.paydetails"/></td>
				<td><html:text property="paydetail" styleClass="formTextFieldWithoutTransparent"></html:text></td>
			</tr>
				
			<tr>
				<td><bean:message key="label.chqno"/></td>
				<td><html:text property="chequeno" styleClass="formTextFieldWithoutTransparent" styleId="chequeno" onkeypress="return onlynumbers()"></html:text></td>
			</tr>
				
			<tr>
				<td><bean:message key="label.chqdate"/></td>
				<td><html:text property="chequedate" styleClass="formTextFieldWithoutTransparent"  styleId="chqdate"   onblur="checkformat(this)"></html:text></td>
			</tr>
				
			<tr>
				<td><bean:message key="label.chqddpo"></bean:message></td>
				<td><html:select property="chqddpo" styleClass="formTextFieldWithoutTransparent" onblur="select_ChqPo()"  styleId="chddpo" >
					<html:option value="C"><bean:message key="label.Cheque"></bean:message></html:option>
					<html:option value="D"><bean:message key="label.dd"></bean:message></html:option>
					<html:option value="P"><bean:message key="label.po"></bean:message></html:option>
					</html:select>
				</td>
			</tr>
	
			<tr>
				<td><bean:message key="label.bounce"/></td>
				<td><html:checkbox property="bounce" styleClass="formTextFieldWithoutTransparent" styleId="bounce_id" onclick="callBounce()" ></html:checkbox>
					<html:text property="bounceFine" readonly="true" style="color:red" styleClass="formTextField"></html:text></td>
			</tr>
				
			<tr>
				<td><bean:message key="label.discount"/></td>
				<td><html:checkbox property="discount" styleClass="formTextFieldWithoutTransparent" styleId="chk_discount" onclick="check_discount()"></html:checkbox>
				</td>
			</tr>
				
			<tr>
				<td>
				<div id="div_discount1"  style="display: none;">
				<bean:message key="label.dis_amount"/>
				</div>		
				</td>
		
				<td>
				<div id="div_discount" style="display: none;">
				<html:text property="discountamount" styleClass="formTextFieldWithoutTransparent" size="5" styleId="dis_amt" onblur="return setflag(11,this)"  onkeypress="return onlyDoublenumbers()"></html:text>
				<html:text property="discountcharge" styleClass="formTextFieldWithoutTransparent" styleId="dis_chg"  onkeypress="return onlyDoublenumbers()" size="5"></html:text>
				</div>		
				</td>
			</tr>
	
		
			<html:hidden property="form_flag" styleId="flagset"></html:hidden>
			<html:hidden property="reason_codes" styleId="reason"></html:hidden>
			<html:hidden property="error_flag" styleId="err_id"></html:hidden>
			<html:hidden property="today_date" styleId="today_dt"></html:hidden>
			<html:hidden property="booleanFlag"></html:hidden>
		
		</table>
		</td>
		<td>
		<table>
			<tr>
			<td>
			<div id="div_debit_pan" style="display:none;" >  
 			<table style="border:thin solid #000000;">
 					<tr>
 						<td><bean:message key="label.debit_ac_ty"></bean:message></td>
 						<td><html:select property="debit_account_type" styleClass="formTextFieldWithoutTransparent" styleId="debit_acc_type">
							<html:option value="select">Select</html:option>
							<core:forEach var="debit_module"  varStatus="count1" items="${requestScope.Debit_Module_code}" >
							<html:option value="${debit_module.moduleCode}"><core:out value="${debit_module.moduleAbbrv}"></core:out></html:option>
							</core:forEach>
							</html:select> 
						</td>
						<td><bean:message key="label.debit_ac_no"></bean:message></td>
 						<td><html:text property="debit_account_no" onkeypress="return onlynumbers()" styleClass="formTextFieldWithoutTransparent" size="7" styleId="debit_acc_num" onblur="return setflag(5,this)" ></html:text></td>
 					</tr>
 					<tr>
 						<td><bean:message key="label.balance"></bean:message></td>
 						<td><html:text property="balance" readonly="true" styleClass="formTextFieldWithoutTransparent" size="7"></html:text></td>
 						<td><bean:message key="label.sha_balance"></bean:message></td>
 						<td><html:text property="shodowbalance" readonly="true" styleClass="formTextFieldWithoutTransparent" size="7"></html:text></td>
 					</tr>
 					<tr>
 						<td><bean:message key="label.debit_chq_date"></bean:message></td>
 						<td><html:text property="debit_chq_date" styleClass="formTextFieldWithoutTransparent"  styleId="debit_che_date" size="7" onblur="chequeDate(this)"></html:text>
 						</td>
 						<td><bean:message key="label.debit_chq_no" ></bean:message></td>
 						<td><html:text property="debit_chequeno" size="7" styleClass="formTextFieldWithoutTransparent" styleId="debit_chq_no" onkeypress="return onlynumbers()" onblur="return setflag(7,this)"></html:text>
 						</td>
 					</tr>
 			</table>
 			</div>
			<div id="po_div" style="display:none;">
			<table style="border:thin solid #000000;">
				<tr>
					<td><bean:message key="label.pay_order_no"/></td>   
					<td><html:text property="pay_order_no" styleClass="formTextFieldWithoutTransparent" size="5" onblur="return setflag(5,this)" ></html:text> 
					</td>
					<td><bean:message key="label.pay_order_date" /></td>
					<td><html:text property="po_date" styleClass="formTextFieldWithoutTransparent" size="5"></html:text>
					</td> 
				</tr>
				<tr>	 
	      			 <td><bean:message key="label.amount" /></td>
			  		 <td><html:text property="pay_order_amt" styleClass="formTextFieldWithoutTransparent" size="5"></html:text>
		  			</td> 	
					<td><bean:message key="label.favour" /></td>
					<td><html:text property="po_favour" styleClass="formTextFieldWithoutTransparent" size="5"></html:text>
					</td> 	
				</tr>
			</table>
			</div>
			<div id="bounce_fine" style="display:none;overflow:scroll;width:310px;height:100px;">
			<table border="1" style="border:thin solid #000000;">
		
				<tr><td>ResCode</td><td>Desptn</td><td>FAmt</td><td>Select</td></tr>
					<% 
						for(int i=0;i<reason.length;i++){ 
					%>
				<tr>
					<td><html:text property="txtReasonCode" size="5" value="<%=""+reason[i].getReasonCd()%>" ></html:text></td>
					<td><html:text property="txtDesription"  value="<%=""+reason[i].getReasonDesc()%>" ></html:text></td>	
					<td><html:text property="txtBounceFine" size="5" value="<%=""+reason[i].getBounceFine()%>" ></html:text></td>
					<td><input type="checkbox" name="chkBox"  value="<%=""+i%>"/></td>
				</tr>
					<%} %>	
			</table>
 			</div>
 		</td>
 		</tr>
 		<tr>
 			<td>
 				<table>
 					<tr>
 					<td>
 							<%  String jspPath=(String)request.getAttribute("flag");
						     	System.out.println("'m inside panel"+jspPath);
						         
						     	if(jspPath!=null){
						            System.out.println("wen 'm  null");
            				%>
            				<jsp:include page="<%=jspPath.toString().trim()%>"></jsp:include>
            
            				<%		}else{	%>
                      		<jsp:include page="/LockerWebPages/blank.jsp"></jsp:include>
          					<%} %>
 					</td>
 					</tr>
 				</table>
 			</td>
 		</tr>
 		</table>
 		</td>
		</tr>
				
 	</table> 
 				<%if(access!=null&&accesspageId!=null&&access.toCharArray()[1]=='1'){%>
             <html:button property="submits" styleClass="ButtonBackGroundImage" onclick="return  setflag(8,this)" value="Submit"></html:button>
             <%}else{ %>
             <html:button property="submits" styleClass="ButtonBackGroundImage" disabled="true" onclick="return  setflag(8,this)" value="Submit"></html:button>
             <%} %>
 				
 				<%if(access!=null&&accesspageId!=null&&access.toCharArray()[3]=='1'){%>
            <html:button property="submits" styleClass="ButtonBackGroundImage" onclick="return setflag(9,this)" value="Update"></html:button>
             <%}else{ %>
           <html:button property="submits" styleClass="ButtonBackGroundImage" disabled="true" onclick="return setflag(9,this)" value="Update"></html:button>
             <%} %>
				
				
				<%if(access!=null&&accesspageId!=null&&access.toCharArray()[2]=='1'){%>
            <html:button property="submits" styleClass="ButtonBackGroundImage" onclick="return setflag(10,this)" value="Delete"></html:button>
             <%}else{ %>
            <html:button property="submits" styleClass="ButtonBackGroundImage" disabled="true" onclick="return setflag(10,this)" value="Delete"></html:button>
             <%} %>
				
				<html:button property="clear"  onclick="clearAll()" styleClass="ButtonBackGroundImage"  value="Clear"></html:button>
</html:form>	
<%}else{ %>
        <font color="red"><bean:message key="label.accessdenied"></bean:message></font>
        <%} %>

</body>
</html>