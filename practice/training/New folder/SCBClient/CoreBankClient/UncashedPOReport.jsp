...
<h2 class="h2">Uncashed PayOrder's Report</h2></center></font>
 ...
 function datevalidation(ids){
  ...
  <body class="Mainbody">
    ...
    <font color="red"><%=request.getAttribute("closingmsg")%></font>
     ...
     <html:form action="/FrontCounter/ListPOUncash?pageId=3050" focus="<%=(String)request.getAttribute("FocusTo") %>" >
     	...
     	<br><br><br>
            ...
            <td><html:button property="sav" value="Download" onclick="setval()" styleClass="ButtonBackgroundImage"></html:button>  </td>
            </tr>
            </table>
            <br><br><br>
            ...
            <td><%=k %></td>
            <td><%=poObj[i].getPayOrderNo() %></td>
             <td><%=poObj[i].getPODate() %></td>
             <td><%=poObj[i].getPOChqNo() %></td>
             <td><%=poObj[i].getPOPayee()%></td>
             <td><%=poObj[i].getTransAmount() %></td>
